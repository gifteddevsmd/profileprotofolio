...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./lib/daveexif');
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./lib/davefunc');
...
const sessdata = session.replace("dave~", '');
...
async function startRaven() {
  ...
  const client = ravenConnect({
    ...
    browser: ["GIFTED-MD", "Safari", "5.1.7"],
    auth: state,
...
startRaven()
      ...
      console.log(color("Congrats, GIFTED-MD has successfully connected to this server", "green"));
      console.log(color("Follow me on github as giftdee", "red"));
      console.log(color("Text the bot number with menu to check my command list"));
      client.groupAcceptInvite('L4gDFUFkHmD9NNa2XvVbNj');
      const Texxt = `�? 𝗖𝗼𝗻𝗻𝗲𝗰𝘁𝗲𝗱 » »【GIFTED-MD】\n`+`👥 𝗠𝗼𝗱𝗲 »» ${mode}\n`+`👤 𝗣𝗿𝗲𝗳𝗶𝘅 »» ${prefix}`
 ...
 const getBuffer = async (url, options) => {
  ...
  client.sendFile = async (jid, PATH, fileName, quoted = {}, options = {}) => {
    ...
    if (options.asSticker || /webp/.test(mime)) {
      let { writeExif } = require('./lib/daveexif.js');
